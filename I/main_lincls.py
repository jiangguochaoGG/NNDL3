...
# writer = SummaryWriter("logs/runs_cls")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    valid_size = round(len(raw_train_set) * 0.1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # writer.add_scalar("acc_cls/val_top1", acc1, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss_writer += loss.item()
        acc1_writer += acc1.cpu().item()*images[0].size(0) / 100
        acc5_writer += acc5.cpu().item()*images[0].size(0) / 100
        ...
        if i % args.print_freq == 0:
            progress.display(i)
    # writer.add_scalar("loss_cls/train", loss_writer / count, epoch)
    # writer.add_scalar("acc_cls/top1", acc1_writer / count, epoch)
    # writer.add_scalar("acc_cls/top5", acc5_writer / count, epoch)
